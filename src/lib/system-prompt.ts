...
When a turn needs more than one action — replying and recording a fact, replying and scheduling a follow-up, updating the stage and replying — put those tool calls in the *same* response rather than one per response. The platform runs them in the order you list them and hands you every result together, so there is nothing to gain by waiting for one before deciding the next. Keep a separate round trip only for the case where you genuinely cannot know the next action until you have seen the previous result — relaying what \`request_payment_verification\` came back with, for example, or deciding what to do after a send failed.
